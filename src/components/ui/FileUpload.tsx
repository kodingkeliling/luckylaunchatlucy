...
export default function FileUpload({ 
  ...
  const handleConnectGoogle = async () => {
    try {
      ...
      if (!redirectUrl || !process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID) {
      ...
      const messageListener = (event: MessageEvent) => {
        ...
        if (event.data.type === 'GOOGLE_AUTH_SUCCESS') {
          ...
          // Immediately set authenticated to true since we know it succeeded
          setIsAuthenticated(true);
          // Also check auth status after successful login as backup
          setTimeout(() => {
            checkAuthStatusLocal();
          }, 500);
  ...
  return (
    <div className="w-full">
      ...
      {previewUrl && !isUploading ? (
        // Show image preview when file is uploaded successfully
            ...
            {uploading || isUploading ? (
              ...
              <>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="flex items-center space-x-1 px-3 py-1 text-xs bg-red-100 text-red-700 rounded-md hover:bg-red-200 disabled:opacity-50 disabled:cursor-not-allowed mx-auto mb-4"
                >
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                  </svg>
